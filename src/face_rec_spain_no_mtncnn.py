...
# CONFIG
# ==============================
BASE = Path("/home/vlad/Desktop/fcml-face")   # adjust if needed
...
# Different names so we don't overwrite the MTCNN version
OUT_CSV = BASE / "results_spain_no_mtcnn.csv"
CACHE_SUFFIX = "_no_mtcnn"   # used in cache filename
...
# ==============================
# FACE EMBEDDER (NO MTCNN)
# ==============================
class FaceEmbedder:
    """
    Face embedder WITHOUT MTCNN.
    It simply resizes the entire image to (image_size, image_size)
    and feeds it directly to the FaceNet (InceptionResnetV1) model.
    """
    ...
    def __init__(self, device: str = "cpu", image_size: int = 160):
        ...
        # simple transform: resize + to tensor
        self.transform = transforms.Compose([
            transforms.Resize((image_size, image_size)),
            transforms.ToTensor(),
        ])

        # FaceNet backbone (pretrained on VGGFace2)
        self.model = InceptionResnetV1(pretrained="vggface2").eval().to(self.device)

    @torch.no_grad()
    def embed_image(self, img: Image.Image):
        # No face detection: use the whole image
        img = img.convert("RGB")
        tensor = self.transform(img).unsqueeze(0).to(self.device)
        emb = self.model(tensor).cpu().numpy().squeeze(0)
        return emb
...
def build_reference_index(dataset_root: Path, country: str, cache_dir: Path):
    # cache file is different from the MTCNN version
    cache_file = cache_dir / f"{country}_ref_index{CACHE_SUFFIX}.npz"
    if cache_file.exists():
        data = np.load(cache_file, allow_pickle=True)
        return data["embeddings"], data["paths"].tolist(), data["labels"].tolist()

    country_root = dataset_root / country
    embedder = FaceEmbedder(device="cpu", image_size=160)
    ...
    print(f"[REF-noMTCNN] Found {len(ref_imgs)} reference images. Embedding...")
    ...
    print(f"[REF-noMTCNN] Cached to {cache_file}")
    return embeddings, paths, labels


# ==============================
# EVALUATE TEST IMAGES (NO MTCNN)
# ==============================
def evaluate_tests(dataset_root: Path, country: str,
    ...
    rows = []
    print(f"[TEST-noMTCNN] Found {len(test_imgs)} test images. Matching...")
    for p in test_imgs:
        img = Image.open(p)
        emb = embedder.embed_image(img)
        gt = person_from_path(p, country_root)

        # no "no-face" case here, we always embed the whole image
    ...
    print(f"[OUT-noMTCNN] Written to {out_csv}")
    return df
